...
export default function FreelancerContracts() {
  ...
  const { data: contracts, isLoading } = useQuery({
    queryKey: ["freelancer-all-contracts", user?.id],
    queryFn: async () => {
      ...
      if (clientIds.length === 0) return data.map((c) => ({ ...c, client: null }));
      const { data: profiles } = await supabase
        .from("profiles")
        .select("user_id, full_name")
        .in("user_id", clientIds);
      const profileMap = new Map((profiles || []).map((p) => [p.user_id, p]));
      return data.map((c) => ({ ...c, client: profileMap.get(c.client_id) || null }));
    },
    enabled: !!user,
  });
      ...
      {isLoading ? (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground mt-1">
                      <span className="flex items-center gap-1">
                        <User className="h-3 w-3" />Client: {contract.client?.full_name || "Unknown"}
                      </span>
                      <span className="flex items-center gap-1">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <div className="text-right">
                      ...
                      <p className="text-sm font-medium text-success">${(Number(contract.amount) - Number(contract.platform_fee)).toLocaleString()}</p>
                    </div>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            <p className="text-muted-foreground">No contracts yet. Submit proposals to land your first gig!</p>
          </CardContent>
        </Card>
      )}
    </div>
